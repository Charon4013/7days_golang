...
type Call struct {
	...
	Reply         interface{} // reply for the function
...
// receive有bug需要解决, TestClient_Call测试时有概率卡死
func (client *Client) receive() {
	...
	for err == nil {
		...
		// call is not exist
		case call == nil:
			log.Println("	call == nil: ", call == nil)
			err = client.cc.ReadBody(nil)
			log.Printf("		err: %s\n", err)
		// call exist but handled error on service
		case h.Error != "":
	...
	// error occurs, so call terminateCalls
...
// DialHTTP connects to an HTTP RPC server at specified network address listening on the default HTTP RPC path
