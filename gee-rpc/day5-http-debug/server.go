...
func (server *Server) ServeConn(conn io.ReadWriteCloser) {
	...
	var opt Option
	//opt := new(Option)
	log.Println("***json.NewDecoder(conn).Decode(&opt): ", opt, &opt)
	// get Option from conn and decode it
	if err := json.NewDecoder(conn).Decode(&opt); err != nil {
		log.Println("rpc server: options error: ", err, opt, &opt, json.NewDecoder(conn).Decode(&opt))
		return
	}
...
// ServeHTTP implements a http.Handler that answers PRC requests
...
// HandleHTTP registers an HTTP handler for RPC messages on rpc Path
